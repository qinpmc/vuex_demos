<template>
  <div>
    ...
    <div>
      直接调用 $store.commit 派发事件 触发 vuex的 mutations
      <br>
      <button @click="$store.commit('add')">+</button>
      <button @click="$store.commit('reduce')">-</button>
      <hr>mapActions 映射 方法到 vuex的 actions
      <br>
      <button @click="addMethod">+</button>
      <button @click="reduceMethod(3)">-</button>
      <button @click="addAsyncMethod">异步增加</button>
      <br>actions 支持异步，而mutations 不支持异步
      <hr>直接调用 $store.dispatch 派发事件 触发 vuex的 actions
      <br>
